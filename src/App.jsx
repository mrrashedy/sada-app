...
export default function Sada() {
  ...
  useEffect(() => {
    try { const s=localStorage.getItem('sada-sources'); if(s) setSources(JSON.parse(s)); } catch {}
  }, []);
  ...
  useEffect(() => {
    const t=setInterval(()=>{ if(refresh) refresh(); },10000);
    return ()=>clearInterval(t);
  }, [refresh]);
  ...
  const allFeed = liveFeed.length>0 ? liveFeed.map((item,i) => ({
    ...
    lk:      `${Math.floor(((i*7+3)%15)+1)}.${(i*3)%9}K`,
    cm:      `${((i*11+5)%3000)}`,
    sh:      `${Math.floor(((i*5+2)%6)+1)}.${(i*7)%9}K`,
  })) : FEED;

  const sourcedFeed = allFeed.filter(item => {
    const idx=SOURCES.findIndex(s=>s.n===item.s?.n);
  ...
  const resetOnboarding = () => {
    try { localStorage.removeItem('sada-ob-done'); localStorage.removeItem('sada-prefs'); } catch {}
  ...
  return (
    <>
      ...
      <div className="app">
        ...
        <div className="bnav">
          {navItems.map(item=>(
            <button key={item.id} className={`bnav-item ${nav===item.id?'on':''}`} onClick={()=>setNav(item.id)}>
              {item.icon(nav===item.id)}<span>{item.label}</span>
            </button>
          ))}
        </div>
