...
-- ---------- sources ----------
-- `id` is `generated by default as identity` so the worker can insert
-- without supplying an id, while the one-shot SQLite migration script can
-- still pass through the existing SQLite primary keys for continuity.
...
-- ---------- documents ----------
-- Same identity pattern as depth_sources — auto-assigned when the worker
-- inserts, explicit when the migration script back-fills existing rows.
